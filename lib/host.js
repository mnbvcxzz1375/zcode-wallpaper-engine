/**
 * zcode-wallpaper-engine — host half.
 *
 * Bridges the local Wallpaper Engine install into the ZCode workbench (web +
 * desktop). Ported from dsh-plugin-wallpaper-engine's Cordis host; the only
 * structural change is the mount point — Cordis's `ctx.webServer.register` is
 * replaced by `registerWallpaperEngineRoutes(app)`, which mounts the same
 * Node-style route handlers onto a Hono app (see the shim near the bottom of
 * the file).
 *
 * Responsibilities:
 ...
 *   2. Enumerate installed wallpapers of the *portable* kinds:
 *        - type "video"  → the project's `.mp4` (or other media) file
 *        - type "web"    → the project's HTML entry
 *      Scene (native 3D) and Application wallpapers are listed too; scenes are
 *      served as pure-JS rendered frames / extracted videos (see README).
 ...
 *        …plus scene-frame / scene-video / scene-runtime / upload / settings /
 *        transcode routes documented in README.md.
 *
 * The module contributes no model-visible tool and no prompt text. Media is
 * read in-place from the user's own library and served only to the workbench
 * that runs on the same machine — nothing is copied, uploaded or redistributed.
 */
...
/** Steam roots from ZCODE_WE_STEAM_ROOT (comma/semicolon separated, Windows or /mnt paths). */
function steamRootsFromEnv() {
  const raw = process.env.ZCODE_WE_STEAM_ROOT && process.env.ZCODE_WE_STEAM_ROOT.trim();
  if (!raw) return [];
  return raw
    .split(/[,;]/)
    .map((s) => s.trim())
...
function configPath() { return join(homedir(), '.zcode-wallpaper-engine', 'config.json'); }
...
// ── Server-side settings validation (mirror of src/client/types.ts Settings;
//    keep the two in sync) ───────────────────────────────────────────────────
// Canonical field names are the CLIENT ones (rate/glass/ratingFilter/…). Older
// host-shaped configs (playbackRate/blur/contentRatingFilter/…) still load via
// the legacy fallbacks in sanitizeSettings below.
...
/** 字体族白名单（字体自定义）. Mirror of src/client/components/Panel.tsx FONT_FAMILIES. */
...
async function extractSceneVideoFrames(src, time, signal) {
  ...
  for (let i = 0; i < videos.length; i++) {
    ...
    const tmpVideo = join(tmpdir(), 'zcode-we-vid-' + process.pid + '-' + i + (extMatch ? extMatch[1] : '.mp4'));
...
// ZCode has no dsh-better-sidebar equivalent: the workbench draws its own
// chrome. The sidebar-glass control group is therefore always reported
// absent so the client never offers a knob that maps to nothing. Kept as a
// function so the settings route's response shape stays identical to the
// upstream port (callers read `betterSidebar` off /settings).
function isBetterSidebarLoaded(_ctx) {
  return false;
}

/**
 * Sanitize a PUT /settings body into the client's Settings shape
 * (src/client/types.ts). Accepts the current client field names and migrates
 * the older host-shaped names (playbackRate / blur / contentRatingFilter /
 * glassAlpha / interval-minutes / …) so existing config.json files keep working.
 */
function sanitizeSettings(raw) {
  if (!raw || typeof raw !== 'object') return null;
  const o = raw;
  const num = (key, legacyKeys, lo, hi, fallback, map) => {
    const candidates = [key, ...(legacyKeys || [])];
    for (const k of candidates) {
      const v = o[k];
      if (typeof v !== 'number' || !Number.isFinite(v)) continue;
      const n = map ? map(v) : v;
      return clampNum(n, lo, hi, fallback);
    }
    return fallback;
  };
  const rotationGroups = Array.isArray(o.rotationGroups)
    ? o.rotationGroups
        ...
        .map((g) => {
          // Client uses intervalSec (seconds). Legacy host used interval (minutes).
          let intervalSec;
          if (typeof g.intervalSec === 'number' && Number.isFinite(g.intervalSec)) {
            intervalSec = clampNum(g.intervalSec, 5, 86400, 300);
          } else if (typeof g.interval === 'number' && Number.isFinite(g.interval)) {
            intervalSec = clampNum(g.interval * 60, 5, 86400, 300);
          } else {
            intervalSec = 300;
          }
          return {
    ...
    : [];
  // wallpaperOpacity: client is 0..1; a legacy percent value (>1) migrates by /100.
  let wallpaperOpacity = 1;
  if (typeof o.wallpaperOpacity === 'number' && Number.isFinite(o.wallpaperOpacity)) {
    const v = o.wallpaperOpacity > 1 ? o.wallpaperOpacity / 100 : o.wallpaperOpacity;
    wallpaperOpacity = clampNum(v, 0, 1, 1);
  }
  const fontFamilyRaw = o.fontFamily;
  const fontFamily = FONT_FAMILY_VALUES.includes(fontFamilyRaw)
    ? (fontFamilyRaw === 'inherit' ? null : fontFamilyRaw)
    : null;
  return {
    wallpaperId: strOrNull(o.wallpaperId),
    paused: o.paused === true,
    wallpaperBlur: num('wallpaperBlur', [], 0, 60, 0),
    scrim: num('scrim', ['dim'], 0, 1, 0.18),
    wallpaperOpacity,
    brightness: num('brightness', ['backgroundBrightness'], 0.2, 2, 1, (v) => (v > 2 ? v / 100 : v)),
    contrast: num('contrast', ['backgroundContrast'], 0.2, 2, 1, (v) => (v > 2 ? v / 100 : v)),
    saturate: num('saturate', ['backgroundSaturate'], 0, 2, 1, (v) => (v > 2 ? v / 100 : v)),
    border: num('border', [], 0, 1, 0),
    glass: num('glass', ['blur'], 0, 60, 18),
    dim: num('dim', [], 0, 1, 0),
    objectFit: clampStr(o.objectFit, OBJECT_FIT_VALUES, 'cover'),
    flip: o.flip === true,
    rate: num('rate', ['playbackRate'], 0.5, 2, 1),
    fpsCap: FPS_CAP_VALUES.includes(o.fpsCap) ? o.fpsCap : 0,
    accent: hexOrNull(o.accent) || '#7c6cff',
    glassColor: hexOrNull(o.glassColor) || '#ffffff',
    glassOpacity: num('glassOpacity', ['glassAlpha'], 0, 0.6, 0.28, (v) => (v > 0.6 ? v / 100 : v)),
    fontEnabled: o.fontEnabled === true || o.fontCustom === true,
    fontColor: hexOrNull(o.fontColor),
    fontWeight: typeof o.fontWeight === 'number' && Number.isFinite(o.fontWeight)
      ? clampNum(o.fontWeight, 100, 900, 400)
      : null,
    fontFamily,
    cursorColor: hexOrNull(o.cursorColor) || hexOrNull(o.caretColor),
    ...
    ratingFilter: clampStr(o.ratingFilter || o.contentRatingFilter, RATING_VALUES, 'everyone'),
    ...
    rotationGroupId: strOrNull(o.rotationGroupId),
  };
}
...
// ffmpeg resolution: ZCODE_WE_FFMPEG env → a local ./ffmpeg/ffmpeg(.exe) next to
...
// ZCODE_WE_TRANSCODE_TIMEOUT_MS (ms).
const TRANSCODE_TIMEOUT_MS = Number(process.env.ZCODE_WE_TRANSCODE_TIMEOUT_MS) || 15 * 60 * 1000;
...
function transcodeCacheDir() {
  const base = process.env.ZCODE_WE_CACHE_DIR && process.env.ZCODE_WE_CACHE_DIR.trim()
    ? process.env.ZCODE_WE_CACHE_DIR.trim()
...
//   C. lazy download cache ~/.zcode-wallpaper-engine/ffmpeg/ffmpeg[.exe]
//      (pinned single-file ffmpeg-static release asset, magic-byte + size
//      verified, atomic rename; runs once per machine, then cached)
//   env ZCODE_WE_FFMPEG  /  plugin-local ./ffmpeg/     ← explicit overrides
...
// `ZCODE_WE_FFMPEG_URL` replaces the list (user-chosen mirror / self-hosted).
function ffmpegDownloadUrls(asset) {
  const env = process.env.ZCODE_WE_FFMPEG_URL && process.env.ZCODE_WE_FFMPEG_URL.trim();
  if (env) return [env];
  return [
...
async function downloadFfmpegToFile(url, tmp, ctrl, job) {
  const res = await fetch(url, { redirect: 'follow', signal: ctrl.signal, headers: { 'User-Agent': 'zcode-wallpaper-engine' } });
...
async function resolveFfmpeg(job) {
  if (process.env.ZCODE_WE_FFMPEG && process.env.ZCODE_WE_FFMPEG.trim()) {
    return process.env.ZCODE_WE_FFMPEG.trim();
  }
  try {
 ...
 */
const DEFAULT_UPLOAD_DIR = join(homedir(), '.zcode-wallpaper-engine', 'uploads');
function resolveUploadDir() {
  if (process.env.ZCODE_WE_UPLOAD_DIR) return process.env.ZCODE_WE_UPLOAD_DIR;
  const cfg = readConfig();
 ...
 * `ZCODE_WE_CACHE_DIR` overrides the location (tests / power users).
 */
function frameCacheDir() {
  if (process.env.ZCODE_WE_CACHE_DIR && process.env.ZCODE_WE_CACHE_DIR.trim()) {
    return process.env.ZCODE_WE_CACHE_DIR.trim();
  }
...
function customFrameDir() { return ensureDirOnce(join(homedir(), '.zcode-wallpaper-engine', 'overrides')); }
...
/**
 * ZCode integration: this module exports `registerWallpaperEngineRoutes(app)`
 * which mounts the whole HTTP surface onto a Hono app — the same app
 * `@zcode/server` builds in packages/server/src/http.ts (see zcode.patch.yml
 * and scripts/install-to-zcode.mjs). Every handler below is a standard
 * Node http handler (req/res); `registerRoute` bridges it onto Hono through
 * @hono/node-server's `c.env.incoming` / `c.env.outgoing`, so the upstream
 * route code — including Range streaming and `createReadStream().pipe(res)` —
 * ports byte-for-byte without a rewrite.
 */
export const BASE_PATH = BASE;

// 场景帧预热逻辑版本：pkg-extract 合成语义变更时自增（作废预热状态记忆，
// 全库重新比对）。与 scene-frame 缓存键前缀解耦 —— 预热不改键。
const SCENE_PREWARM_LOGIC = 'sf33-prewarm-yfix-filter-v4-puppetquad';
...
async function sceneFramePrewarm() {
  ...
  for (const w of scenes) {
    ...
    const key = 'sf33_' + Buffer.from(w.fileAbs, 'utf8').toString('base64url') + '_' + Math.round(mtime);
...
/**
 * Fallback projection for Hono adapters that do not expose the raw Node
 * `IncomingMessage` / `ServerResponse` pair (anything but @hono/node-server).
 * ZCode always uses node-server, so this path is a safety net: it supports the
 * JSON routes and file serving through a PassThrough sink, and body-reading
 * routes through a real Readable. `req.on('data')` / `stream.pipe(res)` keep
 * working because both objects are genuine Node streams.
 */
function runWithWebShim(handler, c) {
  return new Promise((resolve) => {
    const raw = c.req.raw;
    const headers = {};
    if (raw && raw.headers && typeof raw.headers.forEach === 'function') {
      raw.headers.forEach((v, k) => { headers[k.toLowerCase()] = v; });
    }
    // res: a real PassThrough so createReadStream().pipe(res) works unchanged.
    const res = new PassThrough();
    let statusCode = 200;
    const resHeaders = new Map();
    const sinkChunks = [];
    res.on('data', (chunk) => sinkChunks.push(chunk));
    res.on('end', () => finish());
    res.on('error', () => finish());
    // Handlers assign res.statusCode = <code> directly, so the property must
    // feed the status the shim resolves with — a plain field would leave every
    // 404/415/500 reporting as 200 under non-node-server adapters.
    Object.defineProperty(res, 'statusCode', {
      get: () => statusCode,
      set: (v) => { statusCode = typeof v === 'number' ? v : statusCode; },
      configurable: true,
    });
    // Duplex streams expose writableEnded as a getter-only prototype property;
    // install our own configurable slots over it.
    ...
    res.setHeader = (k, v) => resHeaders.set(String(k).toLowerCase(), String(v));
    res.getHeader = (k) => resHeaders.get(String(k).toLowerCase());
    res.removeHeader = (k) => resHeaders.delete(String(k).toLowerCase());
    const realEnd = res.end.bind(res);
    res.end = (data) => {
      res.writableEnded = true;
      return realEnd(data === undefined ? null : data);
    };
    ...
    function finish() {
      if (settled) return;
      settled = true;
      const body = sinkChunks.length ? Buffer.concat(sinkChunks) : null;
      const out = {};
      for (const [k, v] of resHeaders) out[k] = v;
      resolve(c.body(body, statusCode, out));
    }

    // req: a real Readable so every armBodyIdleTimeout / on('data') contract holds.
    const req = raw && raw.body
      ? Readable.fromWeb(raw.body)
      : Readable.from([]);
    req.method = raw.method;
    req.url = raw.url;
    req.headers = headers;
    // Readable.fromWeb destroys the readable once the web body is fully
    // consumed — normal end of input, not a client hang-up. Only a handler
    // abort (req.destroy mid-request, e.g. a rejected upload) is a 499, and it
    // is distinguished by whether the request body was still in flight.
    let requestBodyDone = false;
    req.on('end', () => { requestBodyDone = true; });
    req.on('close', () => { requestBodyDone = true; });
    const origDestroy = req.destroy ? req.destroy.bind(req) : null;
    req.destroy = () => {
      try { origDestroy(); } catch { /* ignore */ }
      // A handler aborting an unfinished request: respond 499 and drop the body.
      if (!settled && !requestBodyDone) {
        settled = true;
        resolve(c.body(null, 499));
      }
    };

    // The handler owns the response lifecycle: it may end synchronously (the
    // JSON routes) or return before an async callback writes the body (upload
    // ends inside a write-stream callback, scene routes inside pkg extraction).
    // Resolving on the handler's return would truncate every async response to
    // an empty 200, so completion is the stream's 'end'/'error' alone — the
    // same contract the real @hono/node-server adapter upholds.
    try {
...
/**
 * Mount the whole wallpaper-engine HTTP surface onto a Hono app.
 *
 * `app` is the Hono instance (`@hono/server`); routes are registered via
 * `registerRoute` below, which bridges Node-style handlers onto Hono through
 * @hono/node-server's `c.env.incoming` / `c.env.outgoing`.
 *
 * `options.authToken` — when ZCode binds a non-loopback host it gates `/api`
 * and `/ws` behind a token. Pass the same token here and every
 * /wallpaper-engine route inherits the exact same cookie/query gate, so a
 * shared host never exposes the user's wallpaper library to the network.
 *
 * Returns a disposer that tears down routes, kills in-flight ffmpeg jobs and
 * destroys every open media stream — the same lifetime semantics the upstream
 * Cordis fiber had.
 */
export function registerWallpaperEngineRoutes(app, options = {}) {
  if (!app || typeof app.all !== 'function') {
    throw new Error('registerWallpaperEngineRoutes: expected a Hono app');
  }

  // Shared-token gate with ZCode's own middleware (packages/server/src/http.ts
  // hasValidLiteToken): cookie first, then ?token=. Loopback dev keeps working
  // because ZCode does not set a token for localhost hosts.
  const authToken = options.authToken?.trim() || undefined;
  const authCookieName = 'zcode_lite_token';
  if (authToken) {
    app.use(`${BASE}/*`, async (c, next) => {
      ...
      if (url.searchParams.get('token') === authToken) {
        c.header(
          'Set-Cookie',
          `${authCookieName}=${encodeURIComponent(authToken)}; Path=/; HttpOnly; SameSite=Lax`,
        );
        await next();
        return;
      }
      ...
      const hit = cookie
        .split(';')
        .some((part) => part.trim() === `${authCookieName}=${encodeURIComponent(authToken)}`);
      if (hit) { await next(); return; }
      return c.json({ error: 'Unauthorized' }, 401);
    });
  }

  // Bridge a Node-style handler onto Hono. @hono/node-server exposes the raw
  // IncomingMessage/ServerResponse pair in c.env; the handlers keep using
  // req.headers / res.setHeader / stream piping exactly as upstream did.
  function registerRoute(spec) {
    const pattern = spec.kind === 'prefix' ? `${spec.path}/*` : spec.path;
    const disposer = app.all(pattern, async (c) => {
      const env = c.env || {};
      const req = env.incoming;
      const res = env.outgoing;
      if (!req || !res) {
        // Non-node-server Hono adapter: the shim resolves with the handler's
        // real status/body, so return it directly — the sentinel below would
        // make Hono report an empty 200 for every route on this path.
        return runWithWebShim(spec.handler, c);
      }
      try {
      ...
      // The Node-style handlers own the response: they write headers, stream
      // bodies and call res.end() themselves. Tell @hono/node-server the
      // response is already on the wire — otherwise it writes a second empty
      // response over the finished one and throws ERR_HTTP_HEADERS_SENT.
      // (x-hono-already-sent is the adapter's own escape hatch: it checks the
      // header record and skips its writeHead/end pair entirely.)
      return new Response(null, { status: 204, headers: { 'x-hono-already-sent': '1' } });
    });
    return () => { try { app.routes?.delete?.(pattern); } catch { /* ignore */ } };
  }
  ...
  // Keep the thumbnail cache bounded across restarts (one small JPEG per
  // distinct source-content revision).
  ...
  async function buildInventory() {
    ...
    const wallpapers = await Promise.all(all.map(async (w) => {
      ...
      return {
        ...
        // 场景包内独立音频（mp3/ogg 等；内嵌 MP4 场景由 scene-video 承担音轨）。
        // 路由在无音频时 404，客户端 HEAD 探测后决定卡片音乐按钮是否显示。
        sceneAudio: w.type === 'scene' && hasFrame ? `${BASE}/scene-audio/${tokenFor(w.fileAbs)}` : null,
        // 自定义画面（截屏导入）是否已设置 —— 客户端据此显示档位总数与状态。
        hasCustomFrame: w.type === 'scene' ? customIds.has(customIdFromAbs(w.fileAbs)) : false,
      };
    }));
    ...
    // an on-demand ffmpeg-extracted frame via /video-preview.
  ...
  // Upload/remove change the library, so the 3s TTL cache must not keep
  // serving the pre-change list: the picker would briefly omit a freshly
  // uploaded wallpaper (or keep showing a deleted one) until the TTL lapsed.
  ...
  disposers.push(registerRoute({
    ...
    handler: (req, res) => {
      ...
      (async () => {
        ...
        const pngPath = join(dir, key + '.png');
        const jpgPath = join(dir, key + '.jpg');
        const gifPath = join(dir, key + '.gif');
        let servePath = existsSync(pngPath) ? pngPath
          : existsSync(jpgPath) ? jpgPath
            : existsSync(gifPath) ? gifPath : null;
        if (!servePath) {
      ...
      if (!entry) {
        ...
        const pending = (async () => {
          ...
          if (isVideo) {
            ...
            const tmpApng = join(tmpdir(), 'zcode-we-anim-' + process.pid + '-' + Date.now() + '.apng');
            const tmpOut = join(tmpdir(), 'zcode-we-anim-out-' + process.pid + '-' + Date.now() + ext);
            try {
  ...
  //    a sanitized copy in ~/.zcode-wallpaper-engine/config.json. This is what
  ...
  disposers.push(registerRoute({
    ...
    handler: (req, res) => {
      ...
      if (method === 'GET') {
        ...
        json(200, { settings: readSettings(), betterSidebar: isBetterSidebarLoaded(options) });
        return;
      }
      if (method !== 'PUT') {
        res.statusCode = 405; res.end('method not allowed'); return;
...
export default { registerWallpaperEngineRoutes, createWallpaperEngineApp, createStandaloneServer };

/**
 * Build a standalone Hono app carrying only the wallpaper-engine routes.
 * Used by the `zcode-wallpaper-engine serve` CLI and by the tests; ZCode itself
 * mounts the routes onto its own app via registerWallpaperEngineRoutes.
 */
export function createWallpaperEngineApp(options = {}) {
  let Hono;
  try {
    // peerDependency — resolved from the host project (ZCode always ships it).
    Hono = require('hono').Hono;
  } catch {
    throw new Error(
      "zcode-wallpaper-engine: 'hono' is a peer dependency. Install it (ZCode already does) or run `zcode-wallpaper-engine serve` after `pnpm add hono`.",
    );
  }
  const app = new Hono();
  registerWallpaperEngineRoutes(app, options);
  return app;
}

/**
 * Standalone HTTP server (CLI / development). Serves the same routes ZCode
 * serves in-process; open the printed URL to exercise the client half against
 * a live host without a full ZCode checkout.
 */
export async function createStandaloneServer(options = {}) {
  ...
  try {
    ...
    throw new Error(
      "zcode-wallpaper-engine: '@hono/node-server' is required for the standalone server (ZCode already ships it).",
    );
  }
  const app = createWallpaperEngineApp(options);
  const port = options.port || Number(process.env.ZCODE_WE_PORT) || 0;
  const hostname = options.host || '127.0.0.1';
  const server = serve({ fetch: app.fetch, hostname, port }, () => {
    const address = server.address();
    const listenPort = typeof address === 'object' && address ? address.port : port;
    console.log(`zcode-wallpaper-engine: http://${hostname}:${listenPort}${BASE}/inventory`);
  });
  return server;
}
