...
// 避免阻塞 ZCode 服务端事件循环 (大型壁纸渲染数秒~数十秒).
...
// encodePng/encodeApng return Node Buffers whose .buffer is a slice of the
// shared 8 KB pool — transferring that would detach memory the pool still
// hands out. Copy into a standalone ArrayBuffer so the frame leaves the
// worker without a copy on the receive side.
...
try {
  ...
  if (!times || !times.length) {
    ...
    const out = transferable(png);
    parentPort.postMessage({ ok: true, png: out, diff, checked }, [out.buffer]);
  } else {
    ...
    const out = transferable(apng);
    parentPort.postMessage({ ok: true, apng: out }, [out.buffer]);
  }
} catch (e) {
