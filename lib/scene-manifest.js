 ...
 *
 * @module zcode-wallpaper-engine/pkg-extract
 */
...
export function decodePngToRgba(pngBuf) {
    let pos = 8;
    let width = 0;
    let height = 0;
    let colorType = 0;
 ...
 */
export function readPkgEntry(data, entry) {
...
export function parseTex(data) {
    ...
    if (parsed.isAnimatedGif)
        info.frames = parsed.frames;
    return info;
}
...
function decodeDxt5(src, width, height) {
    ...
    for (let by = 0; by < blocksY; by++) {
        for (let bx = 0; bx < blocksX; bx++) {
            ...
            if (a0 > a1) {
                for (let k = 2; k < 8; k++)
                    alphas[k] = (((8 - k) * a0 + (k - 1) * a1) / 7) | 0;
            }
            else {
                for (let k = 2; k < 6; k++)
                    alphas[k] = (((6 - k) * a0 + (k - 2) * a1) / 5) | 0;
                alphas[6] = 0;
                alphas[7] = 255;
            }
...
export function decodeTex(data) {
    const parsed = parseTexInternal(data);
    ...
    switch (parsed.format) {
        case TexFormat.RGBA8888: {
            if (bytes.length < width * height * 4) {
                throw new Error('tex: mipmap size mismatch for RGBA8888 (actual ' + bytes.length + ' < expected ' + width * height * 4 + ')');
            }
        ...
        case TexFormat.R8: {
            if (bytes.length < width * height)
                throw new Error('tex: mipmap size mismatch for R8');
        ...
        case TexFormat.RG88: {
            if (bytes.length < width * height * 2)
                throw new Error('tex: mipmap size mismatch for RG88');
            const rgba = new Uint8Array(width * height * 4);
            for (let i = 0; i < width * height; i++) {
                rgba[i * 4] = bytes[i * 2];
        ...
        default:
            throw new TexUnsupportedError(parsed.format, TEX_FORMAT_NAMES[parsed.format] ?? 'unknown(' + parsed.format + ')', parsed.width, parsed.height);
    }
...
function collectImageObjectTextures(imageObject, readJson) {
    ...
    if (ref.toLowerCase().endsWith('.tex')) {
        ...
        if (materialJson && Array.isArray(materialJson.passes)) {
            for (const pass of materialJson.passes)
                pushTextureList(pass?.textures);
        }
    }
    // per-instance texture overrides
    const instance = imageObject.instance;
    if (instance && typeof instance === 'object')
        pushTextureList(instance.textures);
    return out;
}
...
function tryCompositeMultiLayerScene(scene, access, topCandidate) {
    ...
    const imageObjects = objects.filter((obj) => obj &&
        typeof obj === 'object' &&
    ...
    for (const obj of objects) {
        if (!obj.image || typeof obj.image !== 'string' || obj.image.startsWith('models/util/'))
            continue;
        if (obj.visible && typeof obj.visible === 'object' && obj.visible.value === false)
            continue;
        ...
        if (isLikelyMaskOrHelper(obj.image))
            continue;
        ...
        if (!matJson || !Array.isArray(matJson.passes))
            continue;
        const passes = matJson.passes;
        const texName = passes[0]?.textures?.[0];
        if (!texName || isLikelyMaskOrHelper(texName))
            continue;
        const texPath = access.listTexPaths().find((p) => p.toLowerCase() === texName.toLowerCase() ||
        ...
        if (!decoded || decoded.width < 64 || decoded.height < 64)
            continue;
    ...
    if (layers.length <= 1 || !hasLargeBase)
        return null;
...
function extractSceneMainImageVia(access, label) {
    ...
    const has3dModels = scene.objects.some((obj) => obj && typeof obj === 'object' && typeof obj.model === 'string' && obj.model.length > 0);
    if (has3dModels) {
    ...
    for (const path of candidates) {
        ...
        try {
            ...
            if (err instanceof TexUnsupportedError && path === candidates[0])
                throw err;
            lastError = err;
        }
    }
...
export function extractSceneMainImage(pkgData) {
 ...
 */
export function extractSceneMainImageFromDir(dir) {
...
export function hasSceneVideo(pkgData) {
    try {
...
export function hasSceneVideoFromDir(dir) {
    try {
...
function decompressLz4Block(src, decompressedSize) {
    ...
    while (sp < src.length && dp < decompressedSize) {
        ...
        for (let i = 0; i < litLen; i++)
            dst[dp++] = src[sp++];
        if (sp >= src.length || dp >= decompressedSize)
            break;
...
function decodeDXT5Block(block, offset, out, outOffset, outStride) {
    ...
    if (a0 > a1) {
        for (let i = 1; i <= 6; i++)
            alphaLUT[i + 1] = ((7 - i) * a0 + i * a1) / 7;
    }
    else {
        for (let i = 1; i <= 4; i++)
            alphaLUT[i + 1] = ((5 - i) * a0 + i * a1) / 5;
        alphaLUT[6] = 0;
        alphaLUT[7] = 255;
    }
    ...
    let alphaBits = 0n;
    for (let i = 0; i < 6; i++)
        alphaBits |= BigInt(block[offset + 2 + i]) << BigInt(i * 8);
 ...
 */
export function parseTexToRGBA(buf) {
    if (buf.length < 55)
        return null;
...
function mdlVertexStride(flag) {
    let s = 12;
    if (flag & MDL_FLAG_NORMAL)
        s += 12;
    if (flag & MDL_FLAG_TANGENT)
        s += 16;
    if (flag & MDL_FLAG_EXTRA4)
        s += 4;
    if (flag & MDL_FLAG_SKIN_BLEND)
        s += 16;
    if (flag & MDL_FLAG_SKIN_WEIGHT)
        s += 16;
    if (flag & (MDL_FLAG_UV | MDL_FLAG_UV2))
        s += 8;
    if (flag & MDL_FLAG_UV2)
        s += 8;
    return s;
}
...
export function parseMdl(buf) {
    ...
    if (!Number.isFinite(mdlv) || mdlv < 1)
        return [];
    const dv = new DataView(buf.buffer, buf.byteOffset, buf.byteLength);
    ...
    p += 4;
    if (skinCount < 1 || skinCount > 64 || meshCount < 1 || meshCount > 1024)
        return [];
    const meshes = [];
    for (let m = 0; m < meshCount; m++) {
        ...
        if (vBytes < stride || vBytes > 100000000 || vBytes % stride !== 0 || p + vBytes + 4 > buf.length)
            return meshes;
        ...
        for (let v = 0; v < vCount; v++) {
            ...
            if (meshFlag & MDL_FLAG_TANGENT)
                p += 16;
            if (meshFlag & MDL_FLAG_EXTRA4)
                p += 4;
            if (meshFlag & MDL_FLAG_SKIN_BLEND)
                p += 16;
            if (meshFlag & MDL_FLAG_SKIN_WEIGHT)
                p += 16;
            if (hasUv) {
        ...
        p += 4;
        if (iBytes < 2 || iBytes > 60000000 || p + iBytes > buf.length)
            return meshes;
        ...
        if (useU32) {
            ...
            for (let i = 0; i < iCount; i++)
                arr[i] = dv.getUint32(p + i * 4, true);
            indices = arr;
        }
        else {
            const arr = new Uint16Array(iCount);
            for (let i = 0; i < iCount; i++)
                arr[i] = dv.getUint16(p + i * 2, true);
            indices = arr;
        }
        p += iBytes;
...
function containsEmbeddedScript(value, seen = new Set()) {
...
function buildSceneManifestVia(access, token) {
    ...
    const resourceBase = '/wallpaper-engine/scene-resource/' + token + '/';
    const manifest = {
        width,
        height,
        hasMeteors: false,
    ...
    // scene.json general.clearcolor is WE's actual scene clear color and —
    // unlike the schemecolor user property — lives INSIDE scene.pkg
    // (project.json only exists in loose project dirs).
    if (typeof general?.clearcolor === 'string') {
        manifest.clearColor = parseVec3(general.clearcolor, [0.57, 0.71, 0.81]);
    }
    else if (props?.schemecolor?.value && typeof props.schemecolor.value === 'string') {
    ...
    if (is3D) {
        ...
        if (cam?.paths && Array.isArray(cam.paths) && typeof cam.paths[0] === 'string') {
            ...
            if (firstTf) {
                if (firstTf.eye)
                    eye = parseVec3(firstTf.eye, eye);
                if (firstTf.center)
                    center = parseVec3(firstTf.center, center);
                if (firstTf.up)
                    up = parseVec3(firstTf.up, up);
            }
            ...
            if (pathJson?.paths && pathJson.paths.length > 0) {
                ...
                for (const seg of pathJson.paths) {
                    if (!seg.transforms || seg.transforms.length < 2)
                        continue;
                ...
                if (manifest.cameraPaths.length === 0)
                    delete manifest.cameraPaths;
            }
        }
        manifest.camera = {
            eye,
                ...
                if (m.materialPath) {
                    ...
                    try {
                        ...
                        const pass0 = Array.isArray(matJsonRaw?.passes)
                            ...
                            : undefined;
                        if (pass0) {
                            if (typeof pass0.shader === 'string')
                                shader = pass0.shader;
                            if (pass0.blending === 'additive')
                                additive = true;
                            if (pass0.blending === 'translucent')
                                translucent = true;
                            ...
                            if (Array.isArray(pass0.textures) && pass0.textures.length > 0) {
                                ...
                                if (pass0.textures.length > 1)
                                    texPath2 = resolveTexRef(String(pass0.textures[1]));
                            }
        ...
        for (const obj of scene.objects) {
                    ...
                    if (pass0) {
                        ...
                        if (Array.isArray(pass0.textures)) {
                            for (const t of pass0.textures) {
                                const ref = String(t);
                                if (ref.startsWith('_rt_'))
                                    continue; // render-target refs have no file
                                const want = ref.toLowerCase().replace(/\.tex$/i, '');
                        ...
                        if (usv) {
                            for (const [key, uniformName] of Object.entries(usv)) {
                                ...
                                if (typeof pv === 'string')
                                    userColors[uniformName] = parseVec3(pv, [1, 1, 1]);
                                else if (typeof pv === 'number' && Number.isFinite(pv))
                                    userNums[uniformName] = pv;
                            }
                        }
            ...
            if (typeof obj.particle === 'string') {
                ...
                if (!pj)
                    continue;
                const emitter = Array.isArray(pj.emitter) ? pj.emitter[0] : undefined;
                ...
                if (typeof pj.material === 'string') {
                    ...
                    const pass0 = Array.isArray(matJson?.passes) ? matJson.passes[0] : undefined;
                    const texRef = Array.isArray(pass0?.textures) ? String(pass0.textures[0] ?? '') : '';
                    if (texRef) {
                        const want = texRef.toLowerCase().replace(/\.tex$/i, '');
                        texPath = allTex.find((p) => {
    ...
    if (meteorTexPath)
        manifest.meteorTex = resourceBase + meteorTexPath;
    ...
    if (sparkleTexPath)
        manifest.sparkleTex = resourceBase + sparkleTexPath;
    ...
    const hasReflectionEffect = (obj) => (Array.isArray(obj.effects) ? obj.effects : []).some((e) => typeof e?.file === 'string' &&
        e.file.toLowerCase().includes('effects/reflection'));
    for (const obj of sceneObjects) {
        ...
        if (layerShader !== 'flowimage' && isLikelyMaskOrHelper(texName))
            continue;
        const resolveLayerTex = (ref) => allTex.find((p) => p.toLowerCase() === ref.toLowerCase() ||
        ...
        if (!lw || !lh)
            continue;
        if (!decoded && !access.readFile(texPath))
            continue;
        if (decoded && !modelJson.width && decoded.width < 64 && decoded.height < 64)
            continue;
        ...
        const isGround = nameLower.includes('land') ||
...
function extractSceneResourceVia(access, subpath) {
    ...
    const file = access.readFile(norm) ||
...
export function buildSceneManifest(pkgData, token) {
    return buildSceneManifestVia(pkgSceneAccess(pkgData), token);
}
export function buildSceneManifestFromDir(dir, token) {
    return buildSceneManifestVia(dirSceneAccess(dir), token);
}
export function extractSceneResource(pkgData, subpath) {
    return extractSceneResourceVia(pkgSceneAccess(pkgData), subpath);
}
export function extractSceneResourceFromDir(dir, subpath) {
